...
(async function () {
    ...
    function generateOrders() {
        for (let id = 1; id <= 500; id++) {
            ...
            const customersInCity = data.filter((x) => x.model === 'Customer' && x.data.city === city);
            const couriersInCity = data.filter((x) => x.model === 'Courier' && x.data.city === city);
            const restaurantsInCity = data.filter((x) => x.model === 'Restaurant' && x.data.city === city);

            const Customer = customersInCity[Math.floor(Math.random() * customersInCity.length)];
            const Courier = couriersInCity[Math.floor(Math.random() * couriersInCity.length)];
            const Restaurant = restaurantsInCity[Math.floor(Math.random() * restaurantsInCity.length)];
